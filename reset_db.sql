...
DROP TABLE IF EXISTS drop_beats CASCADE;
...
-- Studio table is removed as it's no longer used.
...
create table if not exists cart_items (
  ...
  beat_id uuid references beats(id) on delete cascade not null,
  license_type text not null check (license_type in ('mp3', 'wav', 'stems')),
...
create policy "Allow public insert to cart_items" on cart_items for insert to public with check (true);
create policy "Allow public read of own session cart_items" on cart_items for select to public using (true);
create policy "Allow public delete of own session cart_items" on cart_items for delete to public using (true);
...
drop policy if exists "Public Access Beats" on storage.objects;
create policy "Public Access Beats" on storage.objects for select using ( bucket_id = 'beats' );

drop policy if exists "Admin Upload Beats" on storage.objects;
